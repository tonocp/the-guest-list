 ...
 * app UI. Shows every furniture sprite at the exact size/placement BoardGrid.vue uses
 * (`w-full h-full`, same floor-dither cell background — every sprite, solo or
 * multi-cell, is drawn at 100% of its cell and relies entirely on its own baked-in
 * `FURNITURE_MARGIN` inset for the gap to the cell edge, see gen-sprites.mjs), so a
 * visual glitch or style tweak can be checked here without generating puzzles until one
 * happens to include the right furniture. Delete this file + its route once the
 * furniture art is settled — see AGENTS.md on not leaving one-off debug scaffolding
 * around past its usefulness. */
