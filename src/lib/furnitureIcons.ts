 ...
 * 1 cell, and for `rug`/`sofa` on the rare occasion their footprint didn't grow (see
 * `generator/furniture.ts` `growRug`/`growSofa` fallbacks). See
...
/** Only `rug` (straight, up to 2 cells) and `sofa` (straight or L-shaped, up to 3
 ...
 * `missingCorner` to pick which of the 4 baked orientations to use. `rug` never grows
 * to 3 cells, so it has no `L` entry. */
